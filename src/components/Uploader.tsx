...
export default function Uploader({
  ...
  const icon = isDocMode ? (
    <svg viewBox="0 0 24 24" fill="none" className="h-7 w-7">
      <path d="M7 3h7l5 5v13a2 2 0 01-2 2H7a2 2 0 01-2-2V5a2 2 0 012-2z" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
  ...
  return (
    ...
    >
      <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#f4f4f5] text-[#a1a1aa] transition group-hover:bg-[#e8e8ea]">
        {icon}
      </div>
      ...
      <button
        ...
        className="mt-5 inline-flex items-center rounded-xl bg-[#0c0c0d] px-6 py-2.5 text-sm font-medium text-white transition hover:opacity-85 active:scale-[0.97]"
      >
        {btnLabel}
      </button>
      <input ref={inputRef} type="file" accept={acceptProp} multiple={multiple} onChange={handleInputChange} className="hidden" aria-label={btnLabel} />
    </div>
  )
}