...
export default function Toolbar({
  total, selectionCount, estimatedSize, onClearAll, onGenerate, isGenerating, progressInfo,
}: Props) {
  return (
    <div className="mt-6 flex flex-wrap items-center gap-3">
      ...
      <button type="button" onClick={onGenerate} disabled={isGenerating || total === 0} className="btn-primary">
        {isGenerating ? (
          <><svg viewBox="0 0 24 24" fill="none" className="h-4 w-4 animate-spin-slow"><path d="M21 12a9 9 0 11-6.219-8.56" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>{progressInfo || '生成中…'}</>
        ) : (
          <><svg viewBox="0 0 24 24" fill="none" className="h-4 w-4"><path d="M12 4v12m0 0l-4-4m4 4l4-4M4 18h16" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>生成 PDF</>
        )}
      </button>
    </div>
  )
